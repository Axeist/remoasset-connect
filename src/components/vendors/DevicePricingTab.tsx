...
export function DevicePricingTab() {
  ...
  const uniqueBrands = useMemo(() => [...new Set(data.map((d) => d.brand))].sort(), [data]);
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <Button onClick={() => { setEditItem(null); setAddOpen(true); }} className="gap-1.5 shrink-0 h-12">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {hasActiveFilters && (
          <Button variant="ghost" size="sm" onClick={clearFilters} className="gap-1 text-muted-foreground"><X className="h-3.5 w-3.5" /> Clear all</Button>
        )}
      </div>
            ...
            <>
                          ...
                          <TableRow className="cursor-pointer hover:bg-muted/50" onClick={() => setExpandedId(isExpanded ? null : row.id)}>
                            ...
                            <TableCell>
                              <span className="text-xs text-muted-foreground">
                                {highlightText(row.processor, search)} | {highlightText(row.ram, search)} | {highlightText(row.storage, search)}
                              </span>
                            </TableCell>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 border-t">
                  <span className="text-xs text-muted-foreground">Showing {page * pageSize + 1}–{Math.min((page + 1) * pageSize, sorted.length)} of {sorted.length}</span>
  ...
  return (
    <div className="px-4 py-3 space-y-3">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-8 gap-y-2 text-sm">
        ...
        {row.os && <div><span className="text-muted-foreground text-xs">OS</span><p className="font-medium">{highlightText(row.os, search)}</p></div>}
        <div><span className="text-muted-foreground text-xs">Quantity</span><p className="font-medium">{row.quantity}</p></div>
        <div><span className="text-muted-foreground text-xs">Quote Date</span><p className="font-medium">{row.quote_date}</p></div>
      ...
      {addons.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 mt-1">
            {addons.map((a: any, i: number) => <Badge key={i} variant="outline" className="text-xs gap-1">{a.type}: {a.model} x{a.qty}</Badge>)}
          </div>
        </div>
      )}
