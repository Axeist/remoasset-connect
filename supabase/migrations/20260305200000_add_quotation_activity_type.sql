-- Add 'quotation' to allowed activity types
ALTER TABLE public.lead_activities
  DROP CONSTRAINT IF EXISTS lead_activities_activity_type_check;

ALTER TABLE public.lead_activities
  ADD CONSTRAINT lead_activities_activity_type_check
  CHECK (activity_type IN ('call', 'email', 'meeting', 'note', 'whatsapp', 'nda', 'linkedin', 'quotation'));

-- Update the score trigger to include quotation
...
BEGIN
  CASE NEW.activity_type
    ...
    WHEN 'linkedin'   THEN v_points := 4;
    WHEN 'quotation'  THEN v_points := 7;
    ELSE                   v_points := 1;
  END CASE;
